    ...
    def setup(self, stage=None, save=False, test_all=False):
        ...
        if stage == "fit" or stage is None:
            if self.only_source:  # 只加载源域数据
                ...
                self.train_src_ds, self.train_tgt_ds, _ = torch.utils.data.random_split(
                    self.train_src_ds, [train_size, val_size, test_size])
            else:
                self.train_src_ds = ParametersDataset(
                    csv_file=self.source_csv_file,
                    root_dir=self.data_dir,
                ...
                train_size, val_size = int(0.7 * len(self.train_tgt_ds)), int(0.2 * len(self.train_tgt_ds))
                test_size = len(self.train_tgt_ds) - train_size - val_size
                # 在进行第一次数据集划分之前设置随机种子
                torch.manual_seed(114514)  # seed_value是你选择的随机种子值
                self.target_train, self.target_val, _ = torch.utils.data.random_split(self.train_tgt_ds,[train_size,val_size,test_size])
                # self.train_dataset_num = len(self.train_src_ds)
                # self.val_dataset_num = len(self.train_tgt_ds)
            # if self.load_saved:
        ...
        if stage == "test" or stage is None:
            if self.only_source:  # 只加载源域数据
                ...
                # 在进行第一次数据集划分之前设置随机种子
                torch.manual_seed(114514)  # seed_value是你选择的随机种子值
                _, _,self.test_dataset = torch.utils.data.random_split(self.train_tgt_ds,[train_size, val_size, test_size])
                # self.train_dataset_num = len(self.train_src_ds)
                # self.val_dataset_num = len(self.train_tgt_ds)
        #     if self.load_saved:
    ...
    def train_dataloader(self):
        ...
        if self.only_source:
            ...
            src = DataLoader(self.train_src_ds, batch_size=self.batch_size, shuffle=True, num_workers=self.num_workers,
                             pin_memory=True, drop_last=not self.drop_last)
            tgt = DataLoader(self.target_train, batch_size=self.batch_size, shuffle=True, num_workers=self.num_workers,
                             pin_memory=True, drop_last=not self.drop_last)
            # self.train_dataset_num = len(self.train_src_ds)
            # self.val_dataset_num = len(self.train_tgt_ds)
