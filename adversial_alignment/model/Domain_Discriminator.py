...
class AdversarialLoss(nn.Module):
    ...
    def __init__(self, gamma=1.0, max_iter=1000, use_lambda_scheduler=True, **kwargs):
        ...
        if self.use_lambda_scheduler:
            self.lambda_scheduler = LambdaSheduler(self.gamma, self.max_iter)
    ...
    def forward(self, source, target):
        ...
        source_loss, accuracy_source_D = self.get_adversarial_result(source, True, lamb)
        target_loss, accuracy_target_D = self.get_adversarial_result(target, False, lamb)
        adv_loss = 0.5 * (source_loss + target_loss)
        acc_D = 0.5 * (accuracy_source_D + accuracy_target_D)
        return adv_loss, acc_D
    ...
    def get_adversarial_result(self, x, source=True, lamb=1.0):
        ...
        # return loss_adv
        loss_fn = nn.BCEWithLogitsLoss()  # 使用BCEWithLogitsLoss更稳定
        loss_adv = loss_fn(domain_pred, domain_label.float().to(device))

        # 计算分类精度
        domain_pred_binary = (domain_pred > 0.5).long()  # 使用0.5作为二分类的阈值
        accuracy = (domain_pred_binary == domain_label.long().to(device)).float().mean().item()

        return loss_adv, accuracy
...
class DomainClassifier(nn.Module):
    def __init__(self, embed_dim=256, hidden_dim=256, dropout=0.5):
        super().__init__()
